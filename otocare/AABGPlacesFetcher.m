...
#import "AABGPlacesFetcher.h"
#import <CoreLocation/CoreLocation.h>
#import "AABGPlace.h"
#import "AABConstants.h"

@interface AABGPlacesFetcher ()
...
@implementation AABGPlacesFetcher
...
- (void)fetchNearbyLocations:(CLLocationCoordinate2D)coordinate
{
    ...
    dispatch_async(queue, ^{
        ...
        [query appendFormat:@"&key=%@", AABGoogleAPIKey];
        ...
        if (data) {
            ...
            if (dictionary) {
                ...
                for (NSDictionary *dict in placesDictionaries) {
                    AABGPlace *place = [[AABGPlace alloc] initWithDictionary:dict fromGooglePlaces:YES];
    ...
    dispatch_async(queue, ^{
        ...
        if (data) {
            ...
            if (dictionary) {
                ...
                NSArray *placesDictionaries = dictionary[kGPResults];
                for (NSDictionary *dict in placesDictionaries) {
                    AABGPlace *place = [[AABGPlace alloc] initWithDictionary:dict fromGooglePlaces:NO];
                    if (place) [places addObject:place];
                }
